mock_files <- function(counts, features, sample_dir, type = "10x") {
  ...
  # Parse features and barcodes files include the header, 
  ...
  sparse_mat <- Matrix::Matrix(counts, sparse = TRUE)
  matrix_path <- file.path(sample_dir, matrix_fnames[[type]])
  Matrix::writeMM(sparse_mat, matrix_path)
  R.utils::gzip(matrix_path)
}

local_experiment <- function(
  counts,
  features,
  experiment_dir,
  sample_dir,
