...
test_that("harmony integration works", {
  ...
  integrated_scdata <- suppressWarnings(run_dataIntegration(merged_scdata, config = config, scdata_sketches = "", geosketch = F))
  expect_s4_class(integrated_scdata, "Seurat")
})
...
test_that("FastMNN is not working", {
  ...
  expect_error(suppressWarnings(run_dataIntegration(merged_scdata, config = config, scdata_sketches = "", geosketch = F)))
})
...
test_that("SeuratV4 integration works", {
  ...
  merged_scdata <- suppressWarnings(run_dataIntegration(merged_scdata, config = config, scdata_sketches = "", geosketch = F))
...
test_that("PCA is computed when RPCA method is selected within SeuratV4 integration", {
  ...
  expect_message(run_dataIntegration(merged_scdata, config = config, scdata_sketches = "", geosketch = F), "Running PCA")
})
...
test_that("PCA is not computed when CCA method is selected within SeuratV4 integration", {
  ...
  expect_message(run_dataIntegration(merged_scdata, config = config, scdata_sketches = "", geosketch = F), "PCA is not running .*")
})
...
test_that("SeuratV4 integration finds integration anchors using RPCA method, if method in config is RPCA", {
  ...
  expect_message(merged_scdata <- suppressWarnings(run_dataIntegration(merged_scdata, config = config, scdata_sketches = "", geosketch = F)), "Finding integration anchors using RPCA reduction")
...
test_that("SeuratV4 integration finds integration anchors using CCA method, if method in config is CCA", {
  ...
  expect_message(merged_scdata <- suppressWarnings(run_dataIntegration(merged_scdata, config = config, scdata_sketches = "", geosketch = F)), "Finding integration anchors using CCA reduction")
...
test_that("run_dataIntegration calls remove_genes if there are groups to exclude", {
  ...
  expect_message(
    suppressWarnings(run_dataIntegration(merged_scdata, config = config, scdata_sketches = "", geosketch = F)),
...
test_that("integrate_scdata doesn't run geosketch if geosketch is FALSE", {
  c(scdata_list, sample_1_id, sample_2_id) %<-% mock_scdata()
  cells_id <- mock_ids()
  config <- list(
    dimensionalityReduction = list(numPCs = 2),
  ...
  integrated_scdata <- suppressWarnings(integrate_scdata(scdata_list, config, "", cells_id, task_name = "dataIntegration", geosketch = FALSE))$data
  expect_true(is.null(integrated_scdata@misc$geosketch))
})


test_that("integrate_scdata run geosketch if geosketch is TRUE", {
  c(scdata_list, sample_1_id, sample_2_id) %<-% mock_scdata()
  cells_id <- mock_ids()
  merged_scdata <- create_scdata(scdata_list, cells_id)
  config <- list(
  ...
  integrated_scdata <- suppressWarnings(integrate_scdata(scdata_list, config, "", cells_id, task_name = "dataIntegration", geosketch = T, perc_num_cells = 50))$data
  expect_true(integrated_scdata@misc$geosketch)
})


test_that("perform_geosketch generates the correct number of sketches", {
  c(scdata_list, sample_1_id, sample_2_id) %<-% mock_scdata()
  cells_id <- mock_ids()
  merged_scdata <- create_scdata(scdata_list, cells_id)
  config <- list(
  ...
  perc_num_cells <- 5
  sketched_data <- perform_geosketch(merged_scdata, perc_num_cells)
