...
mock_prev_out <- function(samples = "sample_a", counts = NULL) {
  ...
  prev_out$scdata_list <- add_metadata_to_samples(
...
test_that("Integrate scdata is not affected by sample order", {
  ...
  integrated_scdata_rev <- suppressWarnings(integrate_scdata(
    scdata_list_rev,
    config,
    "",
    cells_id,
...
test_that("Integrate scdata filters out cell ids from the cells_id object", {
  ...
  scdata <- suppressWarnings(integrate_scdata(
    scdata_list,
    config,
    "",
    cells_id,
...
test_that("integrate_scdata calls remove_genes if there are groups to exclude", {
  ...
  config <- list(
    dimensionalityReduction = list(
      numPCs = 2,
      excludeGeneCategories = "cellCycle"
    ),
    dataIntegration = list(
      method = "harmony",
      methodSettings = list(
  ...
  expect_message(
    suppressWarnings(integrate_scdata(
      scdata_list,
      config,
      "",
      cells_id,
...
test_that("customize_default_downsampling_config adds geosketch parameters to config for large datasets", {
  config <- list(
    downsampling = list(
      method = "default"
    )
...
test_that("customize_default_downsampling_config adds geosketch parameters to config for large multisample datasets", {
  config <- list(
    downsampling = list(
      method = "default"
    )
...
test_that("customize_default_downsampling_config keeps CELLS_PER_SAMPLE_SKETCHDATA if total is more than MIN_CELLS_SKETCHDATA", {
  config <- list(
    downsampling = list(
      method = "default"
    )
...
test_that("customize_default_downsampling_config doesn't keep more than half the number of cells", {
  config <- list(
    downsampling = list(
      method = "default"
    )
  ...
  # if kept 5000 cells/sample would be 200 * 5000 = 1,000,000 cells kept
  # but should only keep max 50% of cells, so 500,000 cells kept
  num_samples <- 200
