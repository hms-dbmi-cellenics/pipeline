...
construct_qc_config <-
  function(scdata_list,
    ...
    config_mitochondrial <-
      add_custom_config_per_sample(
        get_sample_mitochondrial_config,
...
get_classifier_config <-
  function(scdata,
           ...
           unfiltered_samples) {
    config$enabled <-
      sample_name %in% unfiltered_samples && !disable_qc_filters
...
get_cellsize_config <-
  function(scdata,
           ...
           unfiltered_samples) {
    minCellSize <-
      generate_default_values_cellSizeDistribution(scdata, config)
...
get_sample_mitochondrial_config <-
  function(scdata,
    ...
    default_max_fraction <-
      generate_default_values_mitochondrialContent(scdata, config)
...
get_dblscore_config <-
  function(scdata,
    ...
    probabilityThreshold <-
      generate_default_values_doubletScores(scdata)
...
get_gene_umi_config <-
  function(scdata,
    ...
    config$filterSettings$regressionTypeSettings[[regression_type]]$p.level <-
      p.level
...
get_embedding_config <- function(scdata_list, config) {
  ...
  default_perplexity <-
    config$embeddingSettings$methodSettings$tsne$perplexity
  default_learning_rate <-
    config$embeddingSettings$methodSettings$tsne$learningRate

  config$embeddingSettings$methodSettings$tsne <- list(
    perplexity = min(default_perplexity, min(vapply(
      scdata_list, ncol, integer(1)
    )) / 100),
    learningRate = max(default_learning_rate, min(vapply(
      scdata_list, ncol, integer(1)
    )) / 12)
  )
