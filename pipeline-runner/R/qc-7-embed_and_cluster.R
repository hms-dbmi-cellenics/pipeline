...
embed_and_cluster <- function(scdata, config, sample_id, task_name = 'configureEmbedding') {
  ...
  embeddingPreviewByCellSets <- purrr::map2(
    embeddingPreview,
    unname(scdata.embedding@active.ident[cells_order]),
    function(x, y) {
      append(x, c("cluster" = paste("Cluster", y, sep = " ")))
    }
  )
  ...
  embeddingPreviewBySamples <- purrr::map2(
    embeddingPreview,
...
coloring_samples_and_cluster <- function(scdata) {
  ...
  if ("color_pool" %in% names(scdata@misc)) {
    ...
    color_pool <- c(
      "#e377c2", "#8c564b", "#d62728", "#2ca02c", "#ff7f0e", "#1f77b4", "#f8e71c", "#3957ff", "#d3fe14",
